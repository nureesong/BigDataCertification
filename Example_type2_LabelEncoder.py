...
다음은 백화점 고객의 구매 데이터이다. 
3500명에 대한 학습 데이터를 이용하여 성별 예측 모형을 만든 후, 2482명에 대한 평가용 데이터에 적용하여 성별 예측값(남자일 확률)을 구하시오.
- 데이터 전처리: 범주형 변수에 레이블 인코딩 적용, 결측값 처리
- 모델링: Logistic Regression, Neural Network, Random Forest 비교
...
model = RandomForestClassifier(n_estimators=20, max_depth=3, max_samples=0.1, random_state=999)
model.fit(X_train, y_train)
